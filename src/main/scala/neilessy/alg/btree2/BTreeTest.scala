...
object BTreeTest {
  ...
  def Leaf[K <: Comparable[K],V](entries:List[(K,V)]) = new BTree[K,V](null,entries map { e => ((e._1,e._2,null))})
  val testTree: BTree[String,String] =
    BTree[String,String](
      Leaf(List(("a","-"),("b","-"))),
      List(
        ("c","-",Leaf(List(("d","-"),("e","-"))))
  ...
  val testTree2: BTree[String,String] =
    BTree[String,String](
      Leaf(List(("i","-"),("j","-"))),
      List(
        ("k","-",Leaf(List(("l","-"),("m","-"))))
  ...
  implicit class CInt( val i: Int ) extends Comparable[CInt] {
    def compareTo(o: CInt) = i - o.i
  ...
  def secondTest() {
    println("### test - main")
    var bTree = BTree.empty[CInt,String]()
      .insert(1,"hello")
      .insert(2,"bye")
      .insert(3,"chow")
      .insert(4,"mary")
